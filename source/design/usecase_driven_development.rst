...
- 各ユースケースごとにシーケンス図を作成する
- 静的モデルの整理をする

    - クラスに対して、振る舞いを割り当てていく
...
- アクタ、バウンダリオブジェクト、エンティティオブジェクト で構成する

  - 各オブジェクトを、メッセージ でつなぐ
  - コントローラオブジェクトは、存在しない。（書いても良い）
  - コントローラオブジェクトは、橋渡し的な役割を果たすため
...
- より完璧な 各ユースケースごとのシーケンス図

    - シーケンス図がユースケース記述に合致している
    - 基本コースと代替コースの両方をカバーしている

- より完璧な クラス図

    - 必要な属性と操作が割り当てられていること
...
- これか実装するコンポーネントの振る舞い（関数）に対して、コンポーネント単位の挙動を検証するテストを実装する
...
- コンポーネントの入力と出力をテストする
- コンポーネントを単独で動かすため、入力と出力のシミュレートにスタブと使う

**実施するタイミングと理由**

- 開発フェーズ全体を通じて、ビルド時に実施する
...
- 結合したコンポーネント間のインタフェースや相互作用の誤りを見つけるために行われるテスト
...
- 通信が必要な「外部」システムとの相互運用が正しく行われるかを確認するためのテスト
...
- 機能や振る舞いに対するテスト
- 内部処理を参照せず、コンポーネントの使用を分析してテストケースを作成する

**実施するタイミングと理由**

- 結合テストが完了した後に、システムが仕様通りに動作するかを確認するために実施する
...
- 契約によって定められた要求（機能/非機能）を満たしているかを確認するためのテスト
...
- 開発に加わっていないユーザによって、実際の運用環境でテストを行う

**実施するタイミングと理由**

- 受け入れテスト前に、製品環境に可能なかぎり等しい環境で行う
...
- 要求で指定された限界付近、あるいはそれを超える負荷をかけて、システムのパフォーマンスを確認する
...
- システムに大量のデータを投入して、システムのパフォーマンスを確認する
